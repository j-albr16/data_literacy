...
def radar(
    ...
    theta = radar_factory(len(all_emotions), frame='polygon')
    ...
    for p in surnames:
        df_pol = df[df['surname'] == p].reset_index(drop=True)
        data = df_pol[all_emotions].mean().to_numpy()
        data = data / np.sum(data)

        ax.plot(theta, data, label=p)
        ax.fill(theta, data, alpha=0.25)
        ax.set_varlabels(all_emotions)
        ax.set_ylim(0, 0.4)
        ax.set_rgrids([0.1, 0.2, 0.3, 0.4])
        ax.set_title(f'{p} emotion profile', pad=20)

    ax.legend()
    plt.savefig(f'profile_emotion.png', dpi=300)
    # plt.show()
